...
For this refactor only, coding may be parallel under the disjoint `src/batches/batch-01` through `batch-08` and matching test roots. The 120 field specifications are centralized at `field-specs/tools/<tool_name>.md`; ownership follows the corresponding source/test batch rather than separate field-spec directories. Shared public contracts, registry, governance, staging, status/diff inspection, and commits remain serial. No registry entry may import a nonexistent or unverified batch. No old project, old commit, backup, or `doubao_glm_api_package/` recovery is permitted.
