...
const BODIES: Readonly<Record<P0SourceId, string>> = Object.freeze({
  ...
  float pulseProgress = clamp(u_time / durationSeconds, 0.0, 1.0);
  float pulseLifetime = 2.0 / (pulseCount + 1.0);
  float ring = 0.0;
  float refraction = 0.0;
  for (int i = 0; i < 32; i++) {
    float enabled = 1.0 - step(pulseCount, float(i) + 0.5);
    float emissionStart = float(i) / (pulseCount + 1.0);
    float phase = (pulseProgress - emissionStart) / pulseLifetime;
