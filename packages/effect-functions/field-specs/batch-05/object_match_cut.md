...
用户只需提供前后两张独立图片；服务器静态帧源适配会生成两路受控帧源，并从各自图片派生锁定的亮度轮廓遮罩用于预览对位，不要求用户再上传两张遮罩，不得把 `image` 直接绑定为 `video`。模型不输出资源 ID、路径、URL 或主体选择，资源身份不会进入参数。转场必须同时使用两张图片，并在切点附近完成尺度、旋转和轮廓对齐。本工具不适用于无可匹配主体的素材、语义检测、镜头跟踪、传送门、翻页或多物体组合剪辑。
