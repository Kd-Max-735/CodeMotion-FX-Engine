...
推荐 `duration=0.5..1.4`、`startScale=0.15..0.5`、`endScale=2..5`、`tunnelDepth=4..12`。中性值为 `motionBlur=0.5`、`twistDegrees=0`；完整默认值见示例。用户明确说“导出/输出 N 秒视频”或“视频总时长 N 秒”时，服务器严格使用该时长，允许第二段视频提前结束；未明确总输出时长时，才按两段视频元数据自适应，转场前保留第一段并保证第二段完整播放。`transitionStart` 和 `duration` 都是特效局部时间，不得误判成总输出时长。
