...
相机真实前后移动，同时根据主体距离补偿垂直 FOV，使服务端绑定主体的投影尺寸基本恒定，产生经典空间压缩或扩张感。
...
| 字段 | 必填 | 取值 | 选择策略 |
| --- | --- | --- | --- |
| `direction` | 否 | `forward` / `backward`，默认 `forward` | 决定相机移动方向 |
...
- “前推背景拉开”用 `forward`；“后拉背景压缩”用 `backward`。
...
用户只需提供一张源图片；服务器静态帧源适配会生成受控视频帧，并从画面中心主体区域派生锁定的 camera target，不得把 `image` 直接绑定为 `video`。模型不输出目标坐标、资源 ID、结束 FOV 或第二段运动，资源身份也不会进入参数。`forward` 必须表现为前推并扩大视野，`backward` 必须表现为后拉并压缩背景，不能把两个方向都降级成普通放大。本工具不适用于纯数字 zoom、普通 dolly、无明确主体的镜头、环绕或转场。
