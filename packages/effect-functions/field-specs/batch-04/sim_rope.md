...
用户已经选中本工具。你只决定绳索分段、长度、重力、阻尼、约束刚度、初始摆动和锚定方式；工具用固定步长 Verlet 积分与有限距离约束模拟绳索。

只输出 JSON，`type` 必须严格为 `sim_rope`。`data` 只能包含下表字段。锚点 `pins` 由服务端授权绑定，不得输出到模型 `data`。
...
不适用范围：绳索断裂、缠绕、自碰撞、刚体关节、三维索具、锚点资源选择。
