...
用户已经选中本工具。你只决定布料网格分辨率、尺寸、约束刚度、阻尼、重力、风力和求解次数；工具用固定时间步的 Verlet 积分和结构约束模拟布面。

只输出 JSON，`type` 必须严格为 `sim_cloth`，`data` 只能含下表字段。`mesh` 与 `pins` 都由服务端绑定，不得写入 `data`，也不得输出资源 ID、路径或 URL。
...
不适用范围：撕裂、碰撞体、自碰撞、三维布料材质标定、资源选择。服务端 `mesh` 和 `pins` 不属于模型参数。
