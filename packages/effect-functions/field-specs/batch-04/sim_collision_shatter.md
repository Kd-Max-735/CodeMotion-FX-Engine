...
用户已经选中本工具。你只决定碎片数量、冲击、扩散角、重力、空气阻力、旋转、反弹和随机性；工具把用户的一张图像切成带原图纹理的碎块，再用固定时间步模拟冲量、飞散、旋转和地面碰撞。

只输出 JSON。`type` 必须严格为 `sim_collision_shatter`，`data` 只允许下表字段。`source_image`、`mesh` 与 `fracture_map` 由服务端授权绑定，资源 ID、路径、URL、顶点或破裂图都不能进入模型 `data`。用户只需上传一张 `source_image`。
...
不适用范围：实时断裂拓扑生成、连续碰撞检测、三维网格切割、爆炸烟雾、资源选择。`source_image`、`mesh` 与 `fracture_map` 永远由服务端绑定。
