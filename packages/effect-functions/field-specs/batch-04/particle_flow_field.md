...
用户已经选中本工具。你只决定参数值；工具会以固定时间步将粒子沿程序化或服务端绑定的二维矢量场推进，并计算速度、阻力和湍动。
...
| 字段 | 必填 | 取值 | 选择策略 |
| --- | --- | --- | --- |
| `particleCount` | 否 | 整数 8–128，默认 48 | 控制轨迹密度和计算量 |
...
- “大范围稀疏漂流”：`particleCount` 20，`spawnRadius` 1，`fieldStrength` 1。
- “密集但方向稳定”：`particleCount` 112，`turbulence` 0.1，`drag` 2.4。
...
中性值与默认行为：默认生成 48 个粒子、适中推进和轻微湍动；未提及的维度保持默认，避免把所有参数同时推高。
