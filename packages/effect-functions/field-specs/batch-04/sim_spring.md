...
用户已经选中本工具。你只需决定质量弹簧链的参数；工具用固定步长和受限子步计算弹簧力、阻尼、重力、锚定和初始冲量。

只输出 JSON。`type` 必须严格等于 `sim_spring`，`data` 只能使用下表字段，不得输出解释、资源或其他工具。

合法示例：

```json
{"type":"sim_spring","data":{"nodeCount":16,"stiffness":52,"damping":2.8,"gravity":6,"restLength":0.1,"impulseStrength":2.4,"substeps":3,"anchorMode":"first"}}
```
...
参数优先级：锚定方式 > 软硬 > 重力 > 振荡衰减 > 初始冲量 > 节点数量。`stiffness` 高于 75 时优先把 `substeps` 设为 3–4。
...
中性值与默认行为：默认是 12 节点、单端固定、中等弹性与阻尼、轻度重力和冲量。含糊描述使用默认值。

不适用范围：二维刚体碰撞、布料面、软体体积、真实材料标定、服务端资源选择。
