...
- “完成多少”只映射视频结束时的目标 `progress`，不通过提高网格密度模拟进度；视频时间只负责从 `0` 动画到该目标，不得再次叠加进度。
...
| 细碎像素从左向右依次切换 | `{"grid":64,"order":"linear","seed":1,"progress":0.6}` |
| 大块像素从下向上切换 | `{"grid":8,"order":"linear","seed":4,"progress":0.6}` |
...
默认值为 `grid=20`、`order=random`、`seed=1`、`progress=0.5`。`progress=0/1` 是 A/B 两端，`grid=2` 是最粗网格，`grid=128` 是最细网格；随机模式的种子没有视觉强弱意义，线性模式仅使用方向码 `1..4`。
