...
- `source_layer`（必需，`data`，单个）：服务端绑定的底层画面及其栅格化结果。图片上传模式下使用已授权图片作为底层，图片身份不进入模型参数；缺失时必须停止执行。
- `overlay_layer`（必需，`image`，单个）：服务端绑定且与底层身份独立的上层画面。单图片预览会从同一授权图片派生镜像、调色并带空间 Alpha 变化的上层，使两层都能进入真实混合计算；该派生不创建新的用户资源身份。缺失时必须停止执行。
