...
依据服务端绑定并从一张静态图片派生的深度图，把前景、中景、远景与天空拆成多层空间切片，再移动虚拟相机形成深度相关视差。模型不生成或选择深度图。
...
用户只需提供一张源图片；服务器静态帧源适配会生成受控视频帧，并派生锁定的亮度深度代理用于人工预览分层，不要求再上传一张看不见的深度图片，不得把 `image` 直接绑定为 `video`。模型不能输出深度图、camera target 或任何资源信息。合成必须让近层位移明显大于远层，并让推进和后退产生相反的纵深变化。本工具不适用于真实 3D 网格重建、对象分割、环绕旋转或多特效合成。
