...
用户已经选中本工具。你只决定软体边界节点、弹性、内部压力、阻尼和重力；工具以固定时间步和有限约束迭代模拟可压缩的二维软体，并把用户图像映射到连续形变的软体表面。屏幕坐标中正重力始终向下。

只输出 JSON。`type` 必须严格等于 `sim_soft_body`，`data` 只能含下表参数。`source_image` 与 `mesh` 由服务端授权绑定，绝不能输出到模型 `data`。
...
不适用范围：三维有限元、真实体积材料、布料或绳索拓扑、资源选择。`source_image`、`mesh` 的 ID、路径和顶点数据都不由模型输出。
