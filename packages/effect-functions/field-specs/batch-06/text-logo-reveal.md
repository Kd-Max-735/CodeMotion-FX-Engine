...
将服务端已生成并绑定的文字或 Logo 几何分段，以错峰、位移、翻转和挤出深度完成三维揭示。字体与几何体属于服务端输入，不在 `data` 中出现。

## 服务器资源要求与接入状态

- 必需输入槽：`logo_geometry`，一个 owner-authorized、locked 的 `model`；可选输入槽：`typeface`，一个独立授权的 `font`。同一图片不能冒充几何体或字体。
- 接入状态：**BLOCKED**。等待窗口 12 提供 Logo/文字几何、字体解析与 server-three 真实帧渲染适配器；当前安全失败，不输出揭示元数据假帧。
