...
- 必需输入槽：`source_video`（owner-authorized、locked 的 `video`）和独立的 `subject_tracks`（服务器分析生成的 `data`）。轨迹不能由模型参数或源图伪造。
- 接入状态：**BLOCKED**。等待窗口 12 提供视频解码、主体轨迹解析、时间平滑与裁切帧渲染适配器；当前安全失败，不只返回裁切窗口元数据。
