...
- 必需输入槽：`source_model`，一个 owner-authorized、locked 的 `model`，不得用图片、深度图或不透明占位对象代替。
- 接入状态：**BLOCKED**。等待窗口 12 提供模型切分与 server-three 真实帧渲染适配器；在此之前 `render` 明确返回 `BATCH_06_ADAPTER_REQUIRED`，不会把碎片变换元数据伪装成画面。
