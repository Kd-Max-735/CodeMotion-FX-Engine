...
用户必须上传一份音频，服务端将其绑定到 `audio_analysis` 输入槽（`kind: audio`），解码真实 PCM 并按当前视频时间读取波形窗口。绑定是 owner-scoped、已锁定的精确 `audio-analysis-v1` 对象。该槽不对模型开放；没有真实授权音频时直接拒绝，不生成正弦波、静音线或其他伪波形。
...
模型不能接收音频文件路径、URL、资源 ID 或原始采样；用户上传的音频只由服务器授权、解码和分析。不能生成频谱柱、音高识别或节拍事件，也不组合其他工具。
