...
在用户上传的图片或视频素材上生成具有远近层次的持续降雪或降雨覆盖。雪使用带柔光的飘动薄片，雨使用顺着速度方向拉伸的运动条纹。
...
`background_image` 是必需的服务器授权素材帧，可来自图片或逐帧解码的视频。雨雪只作为覆盖层合成在该素材上，素材身份不进入模型参数；空间分布只由服务器 `seed` 决定。
