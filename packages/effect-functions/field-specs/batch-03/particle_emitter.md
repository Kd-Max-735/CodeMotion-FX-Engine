...
从画面中心持续生成带柔光和高亮核心的粒子，控制速率、初速度、方向、扩散角、生命周期、重力和阻力。粒子纹理由服务端可选绑定；不会附加与粒子无关的扫描线。
...
`particle_texture` 是可选的服务器授权 RGBA 纹理；省略时确定性地使用程序化圆形粒子。绑定存在但格式无效时拒绝，不读取模型参数中的任何素材身份。
