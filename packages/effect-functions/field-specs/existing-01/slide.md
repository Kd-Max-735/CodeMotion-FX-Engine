...
`direction` 与 `distance` 是主运动；非默认 `vector` 才以主距离的四分之一叠加自定义分量。普通左、右、上、下划入保持默认 `vector=[1,0]`，其中上下方向不会产生隐藏的横向位移。只有用户明确要求斜向进入时才修改 `vector`，不要同时用大 `distance` 和大向量重复放大位移。
