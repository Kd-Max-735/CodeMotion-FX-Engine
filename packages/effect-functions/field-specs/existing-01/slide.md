...
| `vector` | 是 | 两元素数组 `[x,y]`，每项 `-2..2`、步长 `0.01`，默认 `[1,0]` | 仅在用户明确要求斜向或额外偏移时调整；正 x 向右，正 y 向下 |

## 参数选择优先级

`direction` 与 `distance` 是主运动；`vector` 只以主距离的四分之一叠加自定义分量。普通左、右、上、下划入保持默认 `vector`，不要同时用大 `distance` 和大 `vector` 重复放大位移。
...
`distance` 推荐值：轻微 `0.15`，中等 `0.35`，明显 `0.7`，强烈 `1.2`。`overshoot` 推荐值：无回弹 `0`，轻微 `0.04`，中等 `0.1`，明显 `0.2`。默认值产生中等距离的左向划入；`distance=0, overshoot=0` 是主要位移的中性设置。
