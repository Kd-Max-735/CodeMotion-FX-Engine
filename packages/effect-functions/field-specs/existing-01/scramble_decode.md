...
让上传图片作为背景，服务端绑定的目标文字先显示为确定性随机字符，再按指定方向逐步锁定为真实字形。对应现有特效 `fx.text.scrambleDecode`。
...
| `text` | 是 | 非空字符串，最长 80 字符，默认 `DECODE` | 最终锁定的原文字内容；服务端使用真实字体栅格化 |
| `fontFamily` | 是 | `song` / `kai` / `sans`，默认 `sans` | 最终文字字体 |
| `fontSize` | 是 | 整数 `12..240` 像素，默认 `72` | 文字字号 |
...
目标文字、字体和字形由服务端绑定。本工具不做加密解密、不生成代码雨或音频，也不要输出文字资源 ID、字体文件路径或 URL。

## 服务器输入槽（不进入模型 `data`）

- `source_image`（必需，`image`，单个）：用户上传并授权的背景图片。乱码文字叠加在该图片上，导出视频保持图片内容，不得使用黑色占位背景。
