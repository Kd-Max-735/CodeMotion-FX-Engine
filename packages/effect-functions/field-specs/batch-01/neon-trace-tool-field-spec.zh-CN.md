...
1. “走到哪里”用 `progress`，“尾巴多长”用 `trailLength`，两者含义不可互换。
...
默认值为示例中的完整 `data`。中性关闭值是 `intensity=0` 或 `glowRadius=0`；稳定动画值是 `pulseRate=0`。不适用于真实灯具照明、文字内容生成、闪电分支或无路径的全屏泛光。路径由服务端绑定，模型只生成上述数值。
